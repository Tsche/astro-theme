/**
 * remark-alert — converts fenced code blocks with `lang="alert"` and
 * GitHub-style blockquote alerts (`> [!NOTE]`) to site callouts.
 *
...
function buildAlertHtml(opts: AlertOptions): string {
  ...
  if (opts.icon !== "none") {
    const iconName =
      opts.icon ?? (opts.type ? DEFAULT_ICONS[opts.type] : undefined);
  ...
  if (hasTitle) {
    ...
    // Override .prose-site h3 margins with inline styles so heading
...
function githubAlertType(node: Readonly<Blockquote>): string | undefined {
  const marker = node.children[0];
  if (!marker || marker.type !== "paragraph") return undefined;
  ...
  return match
    ? GITHUB_ALERT_TYPES[
        match[1].toUpperCase() as keyof typeof GITHUB_ALERT_TYPES
      ]
    : undefined;
}

function isBoldOnlyParagraph(node: Readonly<Paragraph>): boolean {
  return (
...
function spacedBoldTitle(node: Readonly<Paragraph>): string | undefined {
  if (node.children.length !== 1 || node.children[0].type !== "text")
    return undefined;

  // CommonMark deliberately does not parse `** title **` as strong text.
  // Accept it here anyway because it is an unambiguous callout-title intent.
  const match = /^\*\*\s+(.+?)\s+\*\*$/.exec(node.children[0].value.trim());
  return match?.[1];
}
...
// The return type is deliberately erased at the package boundary. Satteri's
// visitor context contains a private field, so two package-manager instances
// are nominally incompatible even when they are the same version.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function satteriAlert(): any {
  return defineMdastPlugin({
    ...
    blockquote(node, ctx) {
      const type = githubAlertType(node);
      if (!type) return;

      // Render the same element/classes as <Callout>. Removing the marker lets
      // the remaining Markdown retain normal rendering (links, emphasis, etc.).
      ...
      if (hasInlineTitle) {
        // Without a blank line after the marker, Markdown keeps the marker and
        // title in one paragraph. Keep the title instead of removing that
        // entire paragraph with the marker.
        const titleNode = firstParagraph.children[1];
        ctx.setProperty(firstParagraph, "children", [titleNode]);
        if (titleNode.type === "strong") {
          ctx.setProperty(firstParagraph, "data", {
            hProperties: { className: ["callout__title"] },
          });
          ctx.setProperty(node, "data", {
            hName: "aside",
            hProperties: {
              className: ["callout", `callout-${type}`, "callout--titled"],
              role: "note",
            },
          });
          return;
        ...
        if (spacedInlineTitle) {
          ctx.setProperty(firstParagraph, "children", [
            { type: "text", value: spacedInlineTitle },
          ]);
          ctx.setProperty(firstParagraph, "data", {
            hProperties: { className: ["callout__title"] },
          });
      ...
      // GitHub alerts can place a blank quoted line between the marker and a
      // bold title. Blank lines do not create MDAST nodes, so the first child
      // after the marker is sufficient for both forms.
      const title = node.children[1] as Paragraph | undefined;
      if (title && isBoldOnlyParagraph(title)) {
        ctx.setProperty(title, "data", {
          hProperties: { className: ["callout__title"] },
        });
        ctx.setProperty(node, "data", {
          hName: "aside",
        ...
        const text = spacedBoldTitle(title);
        if (text) {
          ctx.setProperty(title, "children", [{ type: "text", value: text }]);
          ctx.setProperty(title, "data", {
            hProperties: { className: ["callout__title"] },
          });
          ctx.setProperty(node, "data", {
